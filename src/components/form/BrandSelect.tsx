...
export default function BrandSelect({ value, onChange, error }: BrandSelectProps) {
  ...
  function getVisibleOptions(): (IBrand | { type: "add"; label: string })[] {
    ...
    // Show matching brands + "Add" at bottom if no exact match
    if (showAddNew) {
      result.push({ type: "add", label: `Add "${query.trim()}"` });
    }
    return result;
  }
  ...
  const inp: React.CSSProperties = {
    ...
    color: "#f0f0f4", borderRadius: 6, padding: "8px 12px 8px 34px",
    fontSize: 13, fontFamily: "DM Sans, system-ui, -apple-system, sans-serif", outline: "none",
  };

  return (
    <div style={{ position: "relative" }}>
      <div style={{ position: "relative" }}>
        <Search size={13} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "#6b6b80", pointerEvents: "none" }} />
        <input
          ref={inputRef}
          type="text"
          value={query}
          onChange={handleInputChange}
          onFocus={() => { setIsOpen(true); if (!options.length) loadBrands(""); }}
          onKeyDown={handleKeyDown}
          placeholder="Search or type a brand..."
          autoComplete="off"
          style={inp}
        />
        <ChevronDown size={13} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: "#6b6b80", pointerEvents: "none", opacity: isOpen ? 0.5 : 0.3 }} />
      </div>
      {error && <p style={{ fontSize: 10, color: "#f87171", margin: "3px 0 0", fontFamily: "DM Sans, system-ui, -apple-system, sans-serif" }}>{error}</p>}
      ...
      {isOpen && (
        <div ref={dropdownRef} style={{
          ...
          {visible.map((item, idx) => {
            if ("type" in item) {
              return (
                <div key="add-new" onClick={handleAddNew}
                  onMouseEnter={() => setHighlightIdx(idx)}
                  style={{
                    padding: "9px 14px", cursor: "pointer", fontSize: 12,
                    fontFamily: "DM Sans, system-ui, -apple-system, sans-serif",
                    color: "#2dd4bf", borderTop: "1px solid rgba(255,255,255,0.06)",
            ...
            return (
              <div key={brand.id || brand._id} onClick={() => handleSelect(brand)}
                onMouseEnter={() => setHighlightIdx(idx)}
                style={{
